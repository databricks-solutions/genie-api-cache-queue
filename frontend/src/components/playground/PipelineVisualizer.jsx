...
// Colors: hit = Databricks orange (#FF3621), miss = blue (#2272B4)
function getColors(branchColor, status) {
  if (status === 'completed') {
    if (branchColor === 'orange') return { circle: 'border-[#FF3621] bg-[#FF3621]', line: '#FF3621', label: 'text-[#161616]' }
    if (branchColor === 'blue') return { circle: 'border-[#2272B4] bg-[#2272B4]', line: '#2272B4', label: 'text-[#161616]' }
    return { circle: 'border-[#FF3621] bg-[#FF3621]', line: '#FF3621', label: 'text-[#161616]' }
  }
  if (status === 'active') return { circle: 'border-[#2272B4] bg-white', line: '#2272B4', label: 'text-[#2272B4] font-medium' }
  if (status === 'failed') return { circle: 'border-red-500 bg-red-500', line: '#ef4444', label: 'text-red-600 font-medium' }
  // pending / skipped
  return { circle: 'border-[#EBEBEB] bg-white', line: '#EBEBEB', label: 'text-[#CBCBCB]' }
}
...
function StageNode({ stageId, status, duration, isLast, branchColor }) {
  ...
  else if (status === 'active') icon = <Loader className="w-3 h-3 text-[#2272B4] animate-spin" />
    ...
    <div className="flex items-center">
      ...
      <div className="flex flex-col items-center gap-1 min-w-[72px]">
        ...
        {status === 'completed' && duration != null && (
          <span className="text-[10px] text-[#6F6F6F] -mt-0.5">{formatDuration(duration)}</span>
        )}
        {status === 'active' && (
          <span className="text-[10px] text-[#2272B4] animate-pulse -mt-0.5">...</span>
        )}
      </div>
...
export default function PipelineVisualizer({ stages = [], currentStage, fromCache, error }) {
  ...
  return (
    <div className="bg-white border border-[#EBEBEB] rounded p-4">
      <h3 className="text-[13px] font-medium text-[#161616] mb-3">Query Pipeline</h3>

      {stages.length === 0 && !currentStage ? (
        <div className="py-3 text-center">
          <div className="text-[13px] text-[#6F6F6F] mb-3">Submit a query to see the pipeline</div>
          ...
          <div className="mt-3 flex justify-center gap-5">
            <div className="text-[11px] text-[#CBCBCB] flex items-center gap-1">
